# modify when changing the upstream version
ARG UPSTREAM_RENOVATE_VERSION=37.140.6
...
FROM ${CI_DEPENDENCY_PROXY_GROUP_IMAGE_PREFIX}/renovate/renovate:${UPSTREAM_RENOVATE_VERSION}
# modify when changing the upstream version
ENV UPSTREAM_RENOVATE_VERSION=37.140.6

USER 0
COPY --from=base /app/renovate-0.0.0-semantic-release.tgz /renovate-0.0.0-semantic-release.tgz
RUN npm install --prefix /opt/containerbase/tools/renovate/${UPSTREAM_RENOVATE_VERSION}/ /renovate-0.0.0-semantic-release.tgz
